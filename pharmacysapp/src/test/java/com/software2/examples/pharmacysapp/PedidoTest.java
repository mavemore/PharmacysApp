...
public class PedidoTest {
    ...
    public void testIntegracion_PagoSubtotalHorario() {
        ...
        assertEquals(true, pa.getTipo());//experado,obtenido
        Date fecha = new Date();
        Pedido pedido = new Pedido(carrito,fecha,client);
        System.out.println(pedido.ValidHora());
        assertNotEquals("Sector no válido",pedido.ValidHora());
        System.out.println("----Test 4----\n");
    }
    ...
    public void testIntegracion_Recargo_Total() {
        ...
        Producto p1 = obtener_producto_catalogo(catalogo, "Analgan");
        Producto p2 = obtener_producto_catalogo(catalogo, "Dicloflenaco");
        Producto p3 = obtener_producto_catalogo(catalogo, "Redoxon");
        DetallePedido detalle1 = new DetallePedido(p1, 1);
        DetallePedido detalle2 = new DetallePedido(p2, 5);
        DetallePedido detalle3 = new DetallePedido(p3, 1);
        carrito.add(detalle1);
        carrito.add(detalle2);
        carrito.add(detalle3);
        //visualizar el pedido con los productos seleccionados.
        System.out.println(carrito.toString());
        Pago pa = new Pago();
        //Ingresa el tipo de pago
        pa.crear_pago(true, "");
        Cliente client = new Cliente("Edinson", 1, pa);
        double DELTA = 1e-15;

        double subt = detalle1.subtotal + detalle2.subtotal +detalle3.subtotal;
        System.out.println("El subtotal a pagar es: " + subt);
        //assertEquals(true, pa.getTipo());//experado,obtenido
        Date fecha = new Date();
        Pedido pedido = new Pedido(carrito,fecha,client);
        //System.out.println(pedido.ValidHora());
        //assertNotEquals("Sector no válido",pedido.ValidHora());
        double recargoTest = 0.;
        assertEquals(pedido.GetRecargo(client,subt), recargoTest,DELTA);
        double totalTest = 62.8;
        assertEquals(pedido.TotalPedido(),totalTest,DELTA);
        System.out.println("----Test 5----\n");
    }
